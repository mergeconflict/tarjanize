...
/// Internal target graph used for scheduling.
///
...
pub(crate) struct TargetGraph {
    ...
    /// Predicted compilation cost per target (milliseconds).
    pub costs: Vec<f64>,
...
pub(crate) fn compute_schedule(tg: &TargetGraph) -> ScheduleData {
    let n = tg.names.len();
    if n == 0 {
        return ScheduleData {
            summary: Summary {
    ...
    let total_cost_ms: f64 = tg.costs.iter().sum();
    let symbol_count: usize = tg.symbol_counts.iter().sum();
    let parallelism_ratio = if fwd.critical_path_ms > 0.0 {
        total_cost_ms / fwd.critical_path_ms
    } else {
        0.0
    };
    ...
    ScheduleData {
        summary: Summary {
            critical_path_ms: fwd.critical_path_ms,
            total_cost_ms,
            parallelism_ratio,
            target_count: n,
            symbol_count,
...
fn forward_pass(tg: &TargetGraph, sorted: &[NodeIndex]) -> ForwardResult {
    ...
    let critical_path_ms =
        finish.iter().copied().reduce(f64::max).unwrap_or(0.0);
...
fn backward_pass(
    ...
    let n = tg.names.len();
    let mut longest_from = vec![0.0f64; n];
    ...
    let epsilon = fwd.critical_path_ms * 1e-9;
    let slack: Vec<f64> = (0..n)
        .map(|i| {
            (fwd.critical_path_ms
                - (fwd.start[i] + tg.costs[i] + longest_from[i]))
                .max(0.0)
        })
        .collect();
    let on_critical_path: Vec<bool> =
        slack.iter().map(|&s| s < epsilon).collect();
...
mod tests {
    ...
    fn empty_graph_produces_empty_schedule() {
        ...
        assert!((result.summary.critical_path_ms).abs() < f64::EPSILON);
    ...
    fn single_target_starts_at_zero_with_no_slack() {
        ...
        assert!((t.start).abs() < f64::EPSILON);
        assert!((t.finish - 100.0).abs() < f64::EPSILON);
        assert!((t.cost - 100.0).abs() < f64::EPSILON);
        assert!((t.slack).abs() < f64::EPSILON);
        assert_eq!(t.lane, 0);
        ...
        assert!((result.summary.critical_path_ms - 100.0).abs() < f64::EPSILON);
    ...
    fn chain_all_critical_sequential_starts() {
        ...
        // A: start=0, finish=50
        assert!((result.targets[0].start).abs() < f64::EPSILON);
        assert!((result.targets[0].finish - 50.0).abs() < f64::EPSILON);

        // B: start=50, finish=80
        assert!((result.targets[1].start - 50.0).abs() < f64::EPSILON);
        assert!((result.targets[1].finish - 80.0).abs() < f64::EPSILON);

        // C: start=80, finish=100
        assert!((result.targets[2].start - 80.0).abs() < f64::EPSILON);
        assert!((result.targets[2].finish - 100.0).abs() < f64::EPSILON);

        // All on critical path, zero slack.
        for t in &result.targets {
            assert!(t.on_critical_path, "{} should be critical", t.name);
            assert!(t.slack < 1e-6, "{} should have ~0 slack", t.name);
        }
    ...
    fn fork_parallel_lanes_correct_slack() {
        ...
        assert!((result.targets[0].start).abs() < f64::EPSILON);
        assert!((result.targets[0].finish - 100.0).abs() < f64::EPSILON);

        // B: start=100, finish=130 — on critical path
        assert!((result.targets[1].start - 100.0).abs() < f64::EPSILON);
        assert!((result.targets[1].finish - 130.0).abs() < f64::EPSILON);
        assert!(result.targets[1].on_critical_path);
        assert!(result.targets[1].slack < 1e-6);

        // C: start=100, finish=110 — NOT on critical path
        assert!((result.targets[2].start - 100.0).abs() < f64::EPSILON);
        assert!((result.targets[2].finish - 110.0).abs() < f64::EPSILON);
        // slack = 130 - (100 + 10 + 0) = 20
        assert!(
            (result.targets[2].slack - 20.0).abs() < 1e-6,
            "C slack should be 20, got {}",
            result.targets[2].slack
        );
        assert!(!result.targets[2].on_critical_path);

        // Critical path = A → B
        assert!((result.summary.critical_path_ms - 130.0).abs() < f64::EPSILON);
    ...
    fn diamond_correct_pred_succ() {
        ...
        // A: start=0, finish=10
        assert!((result.targets[0].finish - 10.0).abs() < f64::EPSILON);

        // B: start=10, finish=110
        assert!((result.targets[1].start - 10.0).abs() < f64::EPSILON);
        assert!((result.targets[1].finish - 110.0).abs() < f64::EPSILON);

        // C: start=10, finish=60
        assert!((result.targets[2].start - 10.0).abs() < f64::EPSILON);
        assert!((result.targets[2].finish - 60.0).abs() < f64::EPSILON);

        // D: start=110 (waits for B), finish=130
        assert!((result.targets[3].start - 110.0).abs() < f64::EPSILON);
        assert!((result.targets[3].finish - 130.0).abs() < f64::EPSILON);
        ...
        assert!(
            (result.targets[2].slack - 50.0).abs() < 1e-6,
            "C slack should be 50, got {}",
            result.targets[2].slack
        );
    ...
    fn summary_statistics_correct() {
        ...
        assert!((result.summary.total_cost_ms - 150.0).abs() < f64::EPSILON);
        assert!((result.summary.critical_path_ms - 100.0).abs() < f64::EPSILON);
